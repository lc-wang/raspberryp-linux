...
static int pixpaper_crtc_helper_atomic_check(struct drm_crtc *crtc, struct drm_atomic_state *state);
...
static const uint64_t pixpaper_primary_plane_fmtmods[] = {
					 ...
					 struct drm_atomic_state *state)
{
	...
	if (ret) {
		dev_info(drm->dev, "go to update_cleanup\n");
		goto update_cleanup;
	}
